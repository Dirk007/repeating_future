//! This crate allows you to make any 'async'-function to a [futures::stream::Stream] by yielding the function-futures' result when its ready and calling the function over and over again to endlessly get a new future and yield their results.
//!
//! With the default-enabled feature `streams` this crate provides a handy [FutureStream] which makes a [futures::stream::Stream] out
...
//! support [futures::Stream]. So most examples are made for this for the moment. See the slow_counter example for a minimalistic
//! but totally useless showcase.
